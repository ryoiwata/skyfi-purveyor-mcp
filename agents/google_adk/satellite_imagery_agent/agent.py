...
root_agent = LlmAgent(
    ...
    "\n\n"
    "ARCHIVE IMAGE FIELDS — REQUIRED FORMAT: Whenever you reference an archive image "
    "(from search results, order details, notification history, or any other context), "
    "you MUST present ALL of the following fields for each image. "
    "If a field is not available in the response, show 'N/A'. "
    "Format each archive result like this:\n"
    "  Archive ID: <archive_id>\n"
    "  Preview: <skyfi_preview_url>  ← always use this link, never a raw API URL\n"
    "  Source: <provider>\n"
    "  Sensor: <constellation>\n"
    "  GSD: <gsd>m\n"
    "  Date: <capture_timestamp date>\n"
    "  Local time: <capture_timestamp time>\n"
    "  Cloud cover: <cloud_coverage_percent>%\n"
    "  Off-nadir: <off_nadir_angle>°\n"
    "  Min order size: <min_sq_km> km²\n"
    "NEVER omit any of these fields. NEVER show a raw archive API URL — always use "
    "`skyfi_preview_url` as the clickable preview link. "
    "When listing multiple archives, number each result. "
    "Always prefer `thumbnail_url` for displaying images inline when available. "
    "\n\n"
    ...
    "Archive orders also include `skyfi_preview_url` — share this so users can preview "
    "the imagery. "
